...
#### ProxiDatasets class
class ProxiComparator:

    #### Annotate the input list of spectra
    def compare(self, input_list, resultType='compact', tolerance=None):
        eprint("======= Received input spectra =========")
        eprint(json.dumps(input_list, indent=2, sort_keys=True))
        eprint("============================")

        response = { 'status': {}, "comparison_results": [] }
        ...
        if len(input_list) != 2:
            update_response(response, status='ERROR', status_code=400, error_code='InputListNot2Items', description='Input payload is not list of 2 spectra',
                            log_entry=f"The input payload appears not to be the needed input list of 2 spectra, it has {len(input_list)} items")
            return(response, response['status']['status_code'])
        ...
        for spectrum in input_list:
            ...
            if precursor_mz is None:
                update_response(response, log_entry=f"Entry {i_spectrum - 1} does not have the desirable attribute MS:1000744 - 'selected ion m/z' but will continue anyway without it")
